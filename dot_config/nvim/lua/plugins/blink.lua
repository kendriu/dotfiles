...
return {
	...
	opts = {
		...
		completion = {
			...
			accept = {
				-- Enable auto-brackets for function/method completions
				auto_brackets = {
					enabled = true,
				},
			},
		...
		sources = {
			...
			providers = {
				copilot = {
					...
					-- score_offset raises/lower priority; tweak as needed
					score_offset = 100,
					async = true,
				},
			},
	...
	-- Export LSP capabilities for use in lsp.lua
	-- This keeps all blink configuration in one place
	config = function(_, opts)
		local blink = require("blink.cmp")
		blink.setup(opts)

		-- Make capabilities available globally for LSP configs
		_G.blink_capabilities = blink.get_lsp_capabilities()
	end,
}
